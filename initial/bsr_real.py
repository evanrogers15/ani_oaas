...
def main():
    ...
    if demo_data == 'yes':
        ...
        bsr_real_send_backfill_data(influxdb_token, 30)

        raw_bucket = "bsr_bucket"
        final_bucket = "bsr_final_1m"

        print("Starting InfluxDB calculation creation..")
        bsr_send_flux_task_continous(org_id=org_id, name_value="tixchange", raw_bucket_value=raw_bucket,
                           final_bucket_value=final_bucket, tag_category="bsr_tag", tag_value="app_bsr")

        bsr_send_calculation_backfill(org_id=org_id, name_value="tixchange_backfill", raw_bucket_value=raw_bucket,
                                     final_bucket_value=final_bucket, tag_category="bsr_tag", tag_value="app_bsr", backload_h_r_value=30)
